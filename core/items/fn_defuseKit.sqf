...
while {true} do
{
	...
	if(_cP >= 1 OR !alive player) exitWith {_ui = "tjmsStatusbar" call BIS_fnc_rscLayer;_ui cutRsc["tjmsStatusbar","PLAIN"];};
...
if(!alive player) exitWith {life_action_inUse = false;_ui = "tjmsStatusbar" call BIS_fnc_rscLayer;_ui cutRsc["tjmsStatusbar","PLAIN"];};
