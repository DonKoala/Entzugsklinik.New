...
if((life_veh_shop select 0) == "FICKDICHNEIN!") then {
...
if((life_veh_shop select 0) == "med_airjjjj") then {
	...
	//_vehicle setPos (getMarkerPos _spawnPoint select 0, getMarkerPos _spawnPoint select 1, (getMarkerPos _spawnPoint select 2) + 0.2);
	[[_vehicle,_colorIndex],"life_fnc_colorVehicle",true,false] spawn life_fnc_MP;
	[_vehicle] call life_fnc_clearVehicleAmmo;
	_vehicle setVariable["trunk_in_use",false,true];
	_vehicle setVariable["vehicle_info_owners",[[getPlayerUID player,profileName]],true];
...
// Blinking Lights @by M4rc3lloX
[_vehicle,"M4rc3lloX_blinkingLights",true] spawn life_fnc_vehicleAnimate;
// Blinking Lights @by M4rc3lloX
