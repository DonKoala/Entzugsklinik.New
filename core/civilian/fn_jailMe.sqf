...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		{
			hintSilent format["Zeit verbleibend:\n %1\n",_countDown,[life_bail_amount] call life_fnc_numberText];
		}
		else
		{
			hintSilent format["Zeit verbleibend:\n %1\n",_countDown];
...
switch (true) do
{
	...
	case (_esc) :
	{
		...
		[[0,format["%1 ist aus dem Gefägnis ausgebrochen !",profileName]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
