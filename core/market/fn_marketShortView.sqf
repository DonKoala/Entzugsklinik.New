...
_text = "<t size='1.5' color='#00FF00'>Entzugsklinik Börse</t><br/>";
...

{
	if(_x in life_market_shortnames) then {
		...
		_text = format["%1<br/><t align='left' size='1.2'>%2</t><t align='right' size='1.3' color='#FFFF00'> $%3</t><br/>", _text,_resname, [_price] call life_fnc_numberText];
		
		if(_globalchange >= 0) then
		{
			_text = format["<t align='left'>%1<img size='1' image='icons\trendup.paa'/> $%2</t>", _text, _globalchange];
		}
		else
		{
			_text = format["<t align='left'>%1<img size='1' image='icons\trenddown.paa'/> $%2</t>", _text, _globalchange];
		};
