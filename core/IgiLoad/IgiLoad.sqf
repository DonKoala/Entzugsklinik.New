...
//	Author: Igi_PL edited by M4rc3lloX																							//
...
if (isnil "IL_Variables") then
{
	...
	IL_Para_Smoke_Veh = "SmokeshellOrange";
	IL_Para_Light_Veh = "Chemlight_orange";
	...
	IL_Supported_Vehicles_CHINOOK = ["B_Heli_Transport_03_unarmed_F", "B_Heli_Transport_03_F"];
	...
	IL_Supported_Cargo_Veh_CHINOOK = IL_Supported_Cargo_Veh_Mohawk + IL_Supported_Strider + IL_Supported_Hunter + IL_Supported_Ifrit + IL_Supported_OFFROAD;
...
if (isnil "IL_Procedures") then
{
	...
	IL_Init_Veh =
	{
		...
		if (_obj_type in IL_Supported_Vehicles_CHINOOK) then
		{
			...
			if ((isNil {_obj getVariable "can_copilot"}) || (_force)) then {_obj setVariable["can_copilot", IL_Can_CoPilot, true];};
			if ((isNil {_obj getVariable "can_outside"}) || (_force)) then {_obj setVariable["can_outside", IL_Can_Outside, true];};
			if ((isNil {_obj getVariable "slots_num"}) || (_force)) then {_obj setVariable["slots_num", IL_Num_Slots_CHINOOK, true];}; 	// test
			if ((isNil {_obj getVariable "zload"}) || (_force)) then {_obj setVariable["zload", -2.5, true];}; 							// test
			if ((isNil {_obj getVariable "load_offset"}) || (_force)) then {_obj setVariable["load_offset", 6, true];}; 				// test
			if ((isNil {_obj getVariable "usable_ramp"}) || (_force)) then {_obj setVariable["usable_ramp", IL_Ramp, true];};			// test
