...
while {true} do
{
	...
	if(_cP >= 1 OR !alive player) exitWith {_hideout setVariable["inCapture",false,true];_ui = "tjmsStatusbar" call BIS_fnc_rscLayer;_ui cutRsc["tjmsStatusbar","PLAIN"];};
	if(life_istazed) exitWith {_hideout setVariable["inCapture",false,true];_ui = "tjmsStatusbar" call BIS_fnc_rscLayer;_ui cutRsc["tjmsStatusbar","PLAIN"];}; //Tazed
	if(life_interrupted) exitWith {_hideout setVariable["inCapture",false,true];_ui = "tjmsStatusbar" call BIS_fnc_rscLayer;_ui cutRsc["tjmsStatusbar","PLAIN"];};
};
...
titleText["Das Ganggebiet wurde eingenommen.","PLAIN"];
_flagTexture = [
