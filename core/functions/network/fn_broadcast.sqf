...
if(typeName _type == typeName []) then
{
	for "_i" from 0 to (count _type)-1 do
	{
		switch((_type select _i)) do
		{
			...
			case 3: {[_message,(player getVariable["realname",name player])] call life_fnc_AAN;}; //new line for AAN
	...
	else
{
	switch (_type) do
	{
		...
		case 1: {hint format["%1", _message]};
		case 2: {titleText[format["%1",_message],"PLAIN"];};
		case 3: {[_message,(player getVariable["realname",name player])] call life_fnc_AAN;}; //new line for AAN again.. :D
		case 5: {hint parseText format["%1", _message]};
	};
};