/*
	...
	LUUUKE Come to the Left side of those blinking Lights!
*/
private ["_vehicle","_lightFront","_lightBack","_changeBrightness","_brightness","_attach"];
_vehicle = _this select 0;
if (isNil "_vehicle" || isNull _vehicle || !(_vehicle getVariable "bLeft")) exitWith {};
...
switch (typeOf _vehicle) do {
	...
	case "I_Truck_02_transport_F": { _attach = [[-0.77, 4.13, -1.2], [-0.73, -3.46, -1.22]]; };
	case "I_Truck_02_covered_F": { _attach = [[-0.77, 4.13, -1.2], [-0.73, -3.46, -1.22]]; };
	case "O_Truck_02_fuel_F": { _attach = [[-0.86, 2.78, -1.22], [-0.82, -4.74, -1.22]]; };
	...
	case "B_Truck_01_medical_F": { _attach = [[-0.91, 5.15, -0.46], [-1.28, -5.15, -0.62]]; };
	case "B_Truck_01_fuel_F": { _attach = [[-0.76, 4.99, -0.18], [-0.92, -5.05, -0.58]]; };
	case "B_Truck_01_box_F": { _attach = [[-0.76, 5.28, -0.56], [-1.11, -4.52, -1.255]]; };
	
	case "O_Truck_03_transport_F": { _attach = [[-0.75, 3.4, -1.05], [-0.96, -4.8, -1.15]]; };
	case "O_Truck_03_covered_F": { _attach = [[-0.75, 3.4, -1.05], [-1.01, -4.8, -1.15]]; };
	case "O_Truck_03_device_F": { _attach = [[-0.77, 3.1, -0.79], [-1.03, -5.01, -0.92]]; };
	case "O_Truck_03_ammo_F": { _attach = [[-0.77, 3.1, -0.79], [-1.03, -5.01, -0.92]]; };
};
...
while {(alive _vehicle)} do { 
	if (!(_vehicle getVariable "bLeft")) exitWith {};
	m4rc_blinkerL = true;
	...
	if(m4rc_blinkerL) then {
		if(m4rc_blinkerL && m4rc_blinkerR) then {
